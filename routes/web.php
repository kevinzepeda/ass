...
Route::get('blog/{slug4}/tag', 'BlogController@getBlogTag');
Route::get('blogitem/{slug4?}', 'BlogController@getBlog');
...
Route::group(['middleware' => ['sentinel', 'admin', 'userdata', 'xss_protection'], 'namespace' => 'Admin'], function () {
    ...
    Route::group(['prefix' => 'admin/log_viewer'], function () {
        Route::get('/', 'LogViewerController@index');
        Route::get('logs', 'LogViewerController@listLogs');
        Route::get('logs/{date}', 'LogViewerController@show');
        Route::get('logs/{date}/{level}', 'LogViewerController@showByLevel');
        Route::get('logs/{date}/{level}/search', 'LogViewerController@search');
        Route::get('download/{date}', 'LogViewerController@download');
        Route::delete('logs/delete', 'LogViewerController@delete');
    });

    Route::group(['prefix' => 'admin/blog_category'], function () {
        Route::get('data', 'BlogCategoryController@data');
        Route::get('{blog_category}/delete', 'BlogCategoryController@delete');
    });
    Route::resource('admin/blog_category', 'BlogCategoryController');

    Route::group(['prefix' => 'admin/blog'], function () {
        Route::get('data', 'BlogController@data');
        Route::get('{blog}/show', 'BlogController@show');
        Route::get('{blog}/delete', 'BlogController@delete');
    });
});
Route::group(['middleware' => ['sentinel', 'admin', 'userdata'], 'namespace' => 'Admin'], function () {
    Route::resource('admin/blog', 'BlogController');
    Route::post('admin/blogitem/{blog}/comment', 'BlogController@storeComment');
});
